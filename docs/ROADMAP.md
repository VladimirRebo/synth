...
Synth = личная RAG-система для кодовых баз + VCS-автоматизации.
...
### 1. Каркас бэкенда
- `SYNTH-1` — solution + Web API + `/health` (первая задача в очереди).
- Слоирование конфига (идея `config-layering-cicd`, но для личного окружения — без Kaspi-специфики).
